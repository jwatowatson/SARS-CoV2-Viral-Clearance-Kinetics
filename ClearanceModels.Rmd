...
plot(CT_data$time, CT_data$CtT1,
     ...
     ylab='CT', panel.first=grid(), col=CT_data$Trt_effect+1)
mtext(text = 'B',side = 3,adj = 0,line = 1.5,cex = 1.3)
for(id in unique(CT_data$PersonID)){
  ind = CT_data$PersonID==id
  lines(CT_data$time[ind], 
        CT_data$CtT1[ind], col=adjustcolor(cols[CT_data$Trt_effect[ind]+1],.6))
}
CT_data$Day = round(CT_data$time)

xx = aggregate(CtT1~Day+Trt_effect, CT_data, median)
points(xx$Day[xx$Trt_effect==0], xx$CtT1[xx$Trt_effect==0], pch=17, cex=1.5, col='darkblue')
points(xx$Day[xx$Trt_effect==1], xx$CtT1[xx$Trt_effect==1],
...
xx_rate = aggregate(power ~ effect + N + t_design, 
                    data = res_power, FUN = mean)
xx_time = aggregate(power ~ effect + N + t_design, 
                    data = res_power_TTC,FUN =  mean)

for(td in 1:2){
  ind = xx_rate$t_design==td
  plot(xx_rate$N[ind], xx_rate$power[ind], 
       pch = 14+as.numeric(as.factor(xx_rate$effect[ind])),
       xlab='Number of patients per arm', ylab='Power',
       panel.first=grid(), ylim=c(0,1),cex=1.5)
  abline(h=0.8, lty=2)
  inde = xx_rate$effect==my_effects[1] & ind
  lines(xx_rate$N[inde], xx_rate$power[inde],lwd=1,lty=2)
  inde = xx_rate$effect==my_effects[2] & ind
  lines(xx_rate$N[inde], xx_rate$power[inde],lwd=1,lty=3)
  
  ind = xx_time$t_design==td
  points(xx_time$N[ind], xx_time$power[ind], 
         pch = 14+as.numeric(as.factor(xx_time$effect[ind])),
         col='pink')
  
  inde = xx_time$effect==my_effects[1] & ind
  lines(xx_time$N[inde], xx_time$power[inde],lwd=1,lty=2,col='pink')
  inde = xx_time$effect==my_effects[2] & ind
  lines(xx_time$N[inde], xx_time$power[inde],lwd=1,lty=3,col='pink')
  
  if(td==1){
    legend('bottomright', 
           pch=1,lwd=2, legend = c('Rate','Time to clearance'),
           col=c('black','pink'),
           inset = 0.03)
    mtext(text = 'B - 7 days follow-up',side = 3,adj = 0,line = 1.5,cex = 1.3)

  } else {
    mtext(text = 'C - 5 days follow-up',side = 3,adj = 0,line = 1.5,cex = 1.3)

  }
}

legend('bottomright', 
       pch=15:16, legend = c('30%','50%'),lty=2:3,lwd=1,
       title = expression(paste(beta, ' and ', alpha, ' increase (%)')),
       inset = 0.03)
```

...
plot(CT_data$time, CT_data$log10_vl,
     ...
     ylab='RNA copies per mL', panel.first=grid(), col=CT_data$Trt_effect+1)
...
plot(density(thetas_mod1$intercept), main=expression('A'[0]),
     xlab=expression(paste(Delta,'CT')), ylab='', yaxt='n', xlim = c(4, 6),
...
plot(density(thetas_mod2$intercept[,1]), main=expression('B'[0]),
     ...
     panel.first=grid(),lwd=3,lty=2, xlim = c(1, 4), col=cols[2])

# alpha
plot(density(thetas_mod1$coef), main=expression(alpha), 
     xlab='Fold change per day (copies per mL)',
     ylab='', yaxt='n', xlim = c(0.5,2.5),
     panel.first=grid(),lwd=3, col=cols[1])
...
plot(density(thetas_mod2$coef[,1]), main=expression(beta), 
     ...
     panel.first=grid(),lwd=3,lty=2, xlim = c(.2, .5), col=cols[2])


# sigma 
plot(density(thetas_mod2$sigma_vl), main=expression(sigma),
     xlab='log10 copies per mL', ylab='', 
     yaxt='n', xlim = c(0.6, 1),
...
table(aggregate(power ~ t_design+effect+N,res,mean))
table(aggregate(power ~ t_design+effect+N,res2,mean))
```
